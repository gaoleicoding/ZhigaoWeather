...
(cal: Calendar) {
    ...
    override fun toString(): String {
        return ("农历" + (if (leap) "闰" else "") + chineseNumber[month - 1] + "月"
    ...
    companion object {
        ...
        private fun monthDays(y: Int, m: Int): Int {
            return if (lunarInfo[y - 1900] and 0x10000 != 0L)
                29
            else
                30
        }
        ...
        val lunarDate: String
            get() {
                var lunar: LunarUtil? = null
                try {
                    val date = Date()
                    val dateFormat = SimpleDateFormat("yyyy-MM-dd")
                    val d = dateFormat.format(date)
                    val today = Calendar.getInstance()
                    today.time = chineseDateFormat.parse(d)
                    lunar = LunarUtil(today)
